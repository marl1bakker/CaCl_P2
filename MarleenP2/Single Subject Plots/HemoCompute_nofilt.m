function [HbO, HbR] = HemoCompute_nofilt(DataFolder, SaveFolder, FilterSet, Illumination, b_normalize)
...
if( bSave )
    % Save HbO:    
    % Save .DAT file:
    fidHbO = fopen([SaveFolder 'HbO_nofilt.dat'],'W');
    fwrite(fidHbO, HbO, '*single');
    fclose(fidHbO);    
    fn = setdiff(fieldnames(iFile), {'Properties', 'datFile'});
    
    % Save .MAT file:
    fHbO = matfile([SaveFolder 'HbO_nofilt.mat'], 'Writable', true);
    fHbO.datFile = 'HbO_nofilt.dat';
    for i = 1:numel(fn)
    ...
    % Save HbR:
    % Save .DAT file:
    fidHbR = fopen([SaveFolder 'HbR_nofilt.dat'],'W');
    fwrite(fidHbR, HbR, '*single');
    fclose(fidHbR);
    
    % Save .MAT file:
    fHbR = matfile([SaveFolder 'HbR_nofilt.mat'], 'Writable', true);
    fHbR.datFile = 'HbR_nofilt.dat';
    for i = 1:numel(fn)
