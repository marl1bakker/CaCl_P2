function [HbO, HbR] = HemoComputeNoFilter(DataFolder, SaveFolder, FilterSet, Illumination, b_normalize)
...
f = fdesign.lowpass('N,F3dB', 4, Freq/2, Freq); %Low Pass
lpass_high = design(f,'butter');
f = fdesign.lowpass('N,F3dB', 4, 1/600, Freq); %Low Pass
lpass_low = design(f,'butter'); %tot hier gecomment
...
if( bSave )
    ...
    warning('off')
    delete([SaveFolder 'HbONoFilt.dat']);
    delete([SaveFolder 'HbONoFilt.mat']);
   
    delete([SaveFolder 'HbRNoFilt.dat']);
    delete([SaveFolder 'HbRNoFilt.mat']);
    warning('on')
    
    % Save HbO:    
    % Save .DAT file:
    fidHbO = fopen([SaveFolder 'HbONoFilt.dat'],'W');
    fwrite(fidHbO, HbO, '*single');
    fclose(fidHbO);    
    fn = setdiff(fieldnames(iFile), {'Properties', 'datFile'});
    
    % Save .MAT file:
    fHbO = matfile([SaveFolder 'HbONoFilt.mat'], 'Writable', true);
    fHbO.datFile = 'HbONoFilt.dat';
    for i = 1:numel(fn)
    ...
    % Save HbR:
    % Save .DAT file:
    fidHbR = fopen([SaveFolder 'HbRNoFilt.dat'],'W');
    fwrite(fidHbR, HbR, '*single');
    fclose(fidHbR);
    
    % Save .MAT file:
    fHbR = matfile([SaveFolder 'HbRNoFilt.mat'], 'Writable', true);
    fHbR.datFile = 'HbRNoFilt.dat';
    for i = 1:numel(fn)
