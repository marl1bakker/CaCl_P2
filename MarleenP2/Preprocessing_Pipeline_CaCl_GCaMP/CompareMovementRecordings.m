...
if ~isfile('/home/mbakker/P2_scripts/MarleenP2/RecordingOverview.mat')
    ...
    RecordingOverview = table(Mouse, Folder, A1, A2, A3, Group, Sex, SaveDirectory);
    ...
    for ind = 1:size(Mouse, 1)
        if ~matches(Mouse(ind), 'M1') && ~matches(Mouse(ind), 'M2') %M1 and M2 are special, skip for now
            ...
            if sum(matches(RecordingOverview.Mouse, Mouse(ind))) > 0
                ...
                %             Mouse = Mouse(ind);
                %             Folder = Folder(ind);
                AddMouse = {Mouse(ind), Folder(ind), A1, A2, A3, Group, Sex, SaveDirectory};
...
%% Start movement checking/add A1, A2 and A3 paths
...
for ind1 = 1:size(RecordingOverview.Mouse) %go per mouse
    if isequal(RecordingOverview.A3(ind1), {'empty'}) %assume that if A3 is done, all are done. Don't redo them
        ...
        if (Mousenr >= 15 && Mousenr <= 36)
            RecordingOverview.SaveDirectory{ind1} = '/media/mbakker/GDrive2/P2/GCaMP';
        else
            RecordingOverview.SaveDirectory{ind1} = '/media/mbakker/GDrive/P2/GCaMP';
        end
...
% change locations mice
% load('/home/mbakker/P2_scripts/MarleenP2/RecordingOverview.mat')
% 
% % add column of SaveDirectory
% RecordingOverview.SaveDirectory = cellstr(repmat('empty', size(RecordingOverview,1),1));
% 
% for ind = 1:size(RecordingOverview,1)
%     Mouse = RecordingOverview.Mouse{ind};
%     Mousenr = str2num(Mouse(2:end));
%     
%     if(Mousenr >= 15 && Mousenr <= 36)
%         RecordingOverview.SaveDirectory{ind} = '/media/mbakker/GDrive2/P2/GCaMP';
%     else
%         RecordingOverview.SaveDirectory{ind} = '/media/mbakker/GDrive/P2/GCaMP';
%     end
% end
