function GetAverageCurves_probeersel(DataFolder, averagingtype, datanamefluocurves, datanamehbo, datanamehbr, datanamefluoactivations)
%% set up
...
if ~exist('averagingtype', 'var')
    averagingtype = 'unweighted';
end
...
% Specs = table('Size', [1 4], 'VariableNames', ...
%     {'ROI', 'DelaySec', 'DelayFrames', 'ResponseStrength'}, 'VariableTypes', ...
%     {'cell', 'single', 'single', 'single'});
% Specs.ROI(1) = {'Dummy'};
...
for inddata = 1:size(datanames,2)
    ...
    % save([DataFolder NVCname '.mat'], [datatypes{inddata} 'curves'], '-append');
    %overwrite fluo_nofilt for now, add identifyer to know you overwrote it
    eval([datatypes{inddata} 'curves.unweighted = 1;'])  
    save([DataFolder 'NVC_ROI_fluo_nofilt.mat'], [datatypes{inddata} 'curves'], '-append');
...
%% Get specs
load([DataFolder 'NVC_ROI_fluo_nofilt.mat'], 'fluocurves', 'hbocurves', 'hbrcurves')
...
%% save
save([DataFolder 'NVC_ROI_fluo_nofilt.mat'], 'Specs', '-append');
