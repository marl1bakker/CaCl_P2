...
%     continue
    ...
    if isfield(anaReg.TimecourseFluo, 'ended') ...
            && (isempty(varlist) || ~isfield(anaReg.TimecoursesFluo, 'ended')) %have to check for a way to delete TimecourseFluo...
        anaReg.TimecoursesFluo = anaReg.TimecourseFluo;
    end
...
%             GetTimecourses(SaveFolder, 'centroids', Overwrite, 'hemoCorr_fluo')
...
%             throw(e)
    ...
    if isfield(anaReg.TimecourseHbOHbR, 'ended') ...
            && (isempty(varlist) || ~isfield(anaReg.TimecoursesHbOHbR, 'ended')) %have to check for a way to delete TimecourseFluo...
        anaReg.TimecoursesHbOHbR = anaReg.TimecourseHbOHbR;
    end
    ...
    if( isempty(varlist) || (~isfield(anaReg.TimecoursesHbOHbR, 'ended')) )
        ...
        if isfield(anaReg.HbOHbR, 'ended')
            ...
            try %if yes, try to calculate timecourses
            GetTimecourses(SaveFolder, 'centroids', Overwrite, 'HbO')
            GetTimecourses(SaveFolder, 'centroids', Overwrite, 'HbR')
            Timecoursesvar.ended = datestr(now);
